...
def test_alerts_banner_removed(conn, client):
    """The old yellow IV Rank alerts banner and endpoint are gone."""
    r = client.get("/")
    assert "IV Rank alerts" not in r.text
    assert 'action="/iv-alerts/dismiss"' not in r.text

    # Endpoint no longer exists.
    r2 = client.post("/iv-alerts/dismiss", data={"alert": ["X|2026-07-17T12:00:00"]})
